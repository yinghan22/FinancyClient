<template>
  ...
  <!--  新增  -->
  <el-dialog v-model="create_visible" :closed="get_data(page_info.curr_page)" destroy-on-close title="新增" width="80%">
    <AnnualEconomicBusinessPlanCreateForm :success="create_success"></AnnualEconomicBusinessPlanCreateForm>
  </el-dialog>
  <!--  编辑  -->
  <el-dialog v-model="edit_visible" :closed="get_data(page_info.curr_page)" destroy-on-close title="编辑" width="80%">
    <AnnualEconomicBusinessPlanEditForm :form="data_list[data_index]"
                                        :success="edit_success"></AnnualEconomicBusinessPlanEditForm>
  </el-dialog>
</template>
...
import $$ from '../../../axios'
import AnnualEconomicBusinessPlanCreateForm from './AnnualEconomicBusinessPlanCreateForm.vue'
import AnnualEconomicBusinessPlanEditForm from './AnnualEconomicBusinessPlanEditForm.vue'
...
function data_delete (index) {
  store.commit('loading', true)
  const url = `/aebp/` + data_list.value[index]['id']
  $$.delete(url)
      .then(res => {
        store.commit('loading', false)
        if (res.data.status === 200) {
